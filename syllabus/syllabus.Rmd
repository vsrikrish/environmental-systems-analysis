...
As a student, you may experience a range of issues which can negatively impact your mental health. Please do not ignore any of these stressors, or feel like you have to navigate these challenges alone! The TA and myself are not trained counselors, but we are here to support you in whatever capacity we can. You should never feel that you need to push yourself past your limits to complete any assignment for this class or any other. If we need to make modifications to the course schedule, you can certainly reach out to us. For broader support, please take advantage of [Cornell's mental health resources](https://mentalhealth.cornell.edu/).
...
sched <- rbind(
  ...
  c(11, advdate(semstart, 6, "wed"), "Forest Management", NA),
  c(12, advdate(semstart, 7, "mon"), "Mixed Integer Programming; Solid Waste", NA),
  c(13, advdate(semstart, 7, "wed"), "Air Pollution", "HW2 Due"),
  c(NA, advdate(semstart, 8, "mon"), "\\textbf{Fall Break -- No Class}", "Term Project Release"),
  c(14, advdate(semstart, 8, "wed"), "Optimization with Simulation", "HW3 Assigned"),
  c(15, advdate(semstart, 9, "mon"), "Evolutionary Algorithms", NA),
  c(16, advdate(semstart, 9, "wed"), "Dynamic Programming", NA),
  c(17, advdate(semstart, 10, "mon"), "Robustness", NA),
  c(18, advdate(semstart, 10, "wed"), "Sensitivity Analysis", "HW3 Due"),
  c(19, advdate(semstart, 11, "mon"), "Robustness and SA In Julia", "HW4 Assigned"),
  c(20, advdate(semstart, 11, "wed"), "Multiple Objectives and Trade-Offs", "Term Project Proposal"),
  c(21, advdate(semstart, 12, "mon"), "Decision Trees", NA),
  c(22, advdate(semstart, 12, "wed"), "Stochastic Programming", "HW4 Due"),
  c(23, advdate(semstart, 13, "mon"), "Project Meetings", "HW5 Assigned"),
  c(24, advdate(semstart, 13, "wed"), "Project Meetings", NA),
  c(25, advdate(semstart, 14, "mon"), "Project Meetings", NA),
  ...
  c(28, advdate(semstart, 16, "mon"), "Project Meetings", NA),
...
kable(sched, booktabs = T, format = "latex", escape=F) %>%
  ...
  pack_rows("Systems Management and Optimization", 8, 18, indent=FALSE) %>%
  pack_rows("Analyzing Solutions and Assumptions", 19, 22, indent=FALSE) %>%
  pack_rows("Decision-Making Under Uncertainty", 23, 24, indent=FALSE) %>%
  pack_rows("Term Project Work", 25, 32, indent=FALSE)
```
