...
$$
\begin{align*}
\Rightarrow \frac{dm}{dt} &= m_\text{in} - m + S -D - km \\[0.5em]
...
$$
\begin{align*}
\frac{d(CV)}{dt} &= (u WH) C_\text{in} - (u WH) C(t) + S - D- kC(t)V \\[0.5em]
\frac{d(C)}{dt} &= \frac{u}{L} C_\text{in} - \frac{u}{L} C(t) + S - D - kC(t) \\
\end{align*}
$$

::: {.fragment .fade-in}
$$
\frac{C(t+1) - C(t)}{\Delta t} = \frac{u}{L} C_\text{in} - \frac{u}{L} C(t) + S - D - kC(t)
$$
:::
::: {.fragment .fade-in}
$$
...
$$
C(t+1) = C(t) + \Delta t \left(\frac{{\color{red}u(t)}}{L} \left({\color{red}C_\text{in}(t)} - C(t)\right) + {\color{red}S(t)} - {\color{red}D(t)} - kC(t)\right)
$$
