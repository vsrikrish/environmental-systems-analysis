...
For example: let's say we care about an air pollutant which has a first-order decay rate $k$, so $L(t) = -km(t)$.

$$
\begin{align*}
\Rightarrow \frac{dm}{dt} &= m_\text{in} - m + S - km \\[0.5em]
\dot{m} &= \frac{d(CV)}{dt} = \overbrace{(u WH) C_\text{in}}^{\text{inflow}} - \overbrace{(u WH) C}^{\text{outflow}} + \overbrace{S - D}^{\text{net emissions}} - \overbrace{kCV}^{\text{mass decay}} 
\end{align*}
$$
...
$$
\begin{align*}
\frac{d(CV)}{dt} &= (u WH) C_\text{in}(t) - (u WH) C(t) + S - D- kC(t)V \\[0.5em]
\frac{d(C)}{dt} &= \frac{u}{L} C_\text{in}(t) - \frac{u}{L} C(t) + S - D - kC(t) \\[0.5em]
\frac{C(t+1) - C(t)}{\Delta t} &= \frac{u}{L} C_\text{in}(t) - \frac{u}{L} C(t) + S - D - kC(t)
\end{align*}
$$

$$
\bbox[5px, border: red 5px solid]{C(t+1) = C(t) + \Delta t \left(\frac{u}{L} \left(C_\text{in} - C(t)\right) + S - D - kC(t)\right)}
$$
...
In our prior example, inflow conditions were *static*.

The discretized simulation framework lets us make these time-varying:

$$
C(t+1) = C(t) + \Delta t \left(\frac{u(t)}{L} \left(C_\text{in}(t) - C(t)\right) + S - D - kC(t)\right)
$$
